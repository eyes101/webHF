...
function OrdersTab() {
  ...
  const load = () => {
    setLoading(true);
    api.orders.list(statusFilter || undefined)
  ...
  return (
    <div>
      <div style={{ marginBottom: '20px', display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
        <button className={`btn btn-sm ${!statusFilter ? 'btn-solid' : ''}`} onClick={() => setStatusFilter('')}>All</button>
        {STATUS_OPTIONS.map((s) => (
          <button key={s} className={`btn btn-sm ${statusFilter === s ? 'btn-solid' : ''}`} onClick={() => setStatusFilter(s)}>
            {s.replace('_', ' ')}
          </button>
        ))}
      </div>
